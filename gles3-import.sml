...
struct
  ...
  val (STATIC_DRAW, _) = _symbol "STATIC_DRAW" public: (unit -> update_mode) * (update_mode -> unit);
  val (DYNAMIC_DRAW, _) = _symbol "DYNAMIC_DRAW" public: (unit -> update_mode) * (update_mode -> unit);
  ...
  val bufferData = _import "bufferData" public: Real32.real vector * int * update_mode -> unit;
